...
struct CalorieView : View {
    ...
    var body: some View{
        HStack (alignment: .lastTextBaseline){
            ...
            Spacer()
            Text("\(calories)").font(.system(size: 60 , weight: .bold)).foregroundColor(.white)
            Text("cal").modifier(Tag())
            Spacer()
        
        }.frame(width: .infinity , height: 134 ).background(Color("primary")).cornerRadius(14)
    ...
    var body: some View{
        VStack {
                ...
                HStack {
                        ...
                        Image(systemName: "heart.fill").padding(.leading , 10)
                            .font(.system(size: 36)).foregroundColor(Color("red")).frame(maxWidth: .infinity)
                        
                    
                    HStack(alignment: .lastTextBaseline ){
                        Text("\(Int (heartRate))").font(.system(size: 60 , weight: .bold)).foregroundColor(.white).minimumScaleFactor(0.7)
                        Text("bpm").modifier(Tag())
                        
                    }.frame(maxWidth: .infinity)
  
                }.padding(.top, 10)
            Image("heartRateline").resizable().frame(width: .infinity)
        }.frame(width:.infinity , height: 134 ).background(Color("primary")).cornerRadius(14)
